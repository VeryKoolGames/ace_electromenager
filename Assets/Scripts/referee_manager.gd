...
func _ready() -> void:
	Events.on_player_congratulated.connect(make_referee_congratulate)
	original_panel_position = referee_panel.position

func make_referee_congratulate() -> void:
	if is_on_cooldown:
		return
...
func show_pannel() -> void:
	...
	tween.tween_property(referee_panel, "scale", Vector2.ONE, 0.2).finished.connect(func():
		write_referee_text(congratulations_texts.dialogues_lines[randi_range(0, congratulations_texts.dialogues_lines.size() - 1)])
	)
